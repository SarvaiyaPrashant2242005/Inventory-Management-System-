...
if (isset($_REQUEST['btnEdit'])) {
    $sql = "select * from drone where id = " . $_REQUEST['btnEdit'];
    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <form method="post" action="" class="animate__animated animate__fadeIn">
                <input type="text" name="txtcomponet" placeholder="Enter component" required value="<?php echo $componet ?>">
            ...
            if (isset($_REQUEST['btnAdd'])) {
                ...
                if ($_REQUEST['selid'] == -1)
                    $sql = "insert into drone  (componet,quantities) values  ('$componet', '$quantities ')";
                else
                    $sql  = "update drone set componet = '$componet', quantities='$quantities ' where id = " . $_REQUEST['selid'];
                if (mysqli_query($link, $sql)) {
                    echo '<div class="alert alert-success animate__animated animate__fadeIn" role="alert">Updated Successfully !!!</div>';
                } else {
                    echo '<div class="alert alert-danger animate__animated animate__fadeIn" role="alert">ERROR: Could not execute $sql. ' . mysqli_error($link) . '</div>';
                }
            }


            if (isset($_REQUEST['btnDelete'])) {
                $sql  = "DELETE FROM drone WHERE id = " . $_REQUEST['selid'];
                if (mysqli_query($link, $sql)) {
            ...
            // code to retrieve and display all drones
